<template>
	<view class="uni-card" :class="{ 'uni-card--full': isFull === true || isFull === 'true', 'uni-card--shadow': isShadow === true || isShadow === 'true','uni-border':border }">
		<!-- 基础 -->
		<view v-if="mode === 'basic' && title" class="uni-card__header " :class="{ 'uni-border-bottom':border }" @click.stop="onClick">
			<view v-if="thumbnail" class="uni-card__header-extra-img-view">
				<image :src="thumbnail" class="uni-card__header-extra-img" />
			</view>
			<text class="uni-card__header-title-text font-weight-bold">{{ title }}</text>
			<slot name="extra">
				<text v-if="extra" class="uni-card__header-extra-text" @click="$emit('click-extra')">{{ extra }}</text>
			</slot>
		</view>
		<!-- 标题 -->
		...
		<!-- 内容 -->
		<view class="uni-card__content uni-card__content--pd" :style="bodyStyle" @click.stop="onClick">
			<view v-if="mode === 'style' && extra" class=""><text class="uni-card__content-extra">{{ extra }}</text></view>
			<slot />
		</view>
		<!-- 底部 -->
		<view v-if="note" class="uni-card__footer " :class="{ 'uni-border-top':border }">
			<slot name="footer">
